...
import { geographicOptions, geographicGroups } from "@/lib/constants";
...
export default function Customize({checkResults}:any) {
  ...
  const submit = async () => {
    ...
    // }
    //@ts-ignore
    state && state.length > 0 && setCustomization({...customization, geographic:state });
    setPage(3);
  };
    ...
    <div className="mx-auto w-full max-w-xl">
      <div className="space-y-2">
        ...
        <p className="text-gray-500 dark:text-gray-400">
          Please describe the geographic nature of the area that is facing the issue
        </p>
      </div>
      <div className="space-y-4 pt-8 ">
      <fieldset>
          <div className="grid grid-cols-3 gap-2">
            {geographicGroups.map((option) => (
              <div key={option.name}>
                <Category 
                  image={option.image} 
                  name={option.name}
                  ...
                  select={updateState} 
                  state={state}
                  store={customization.geographic}
                />
              </div>
            ))}
          </div>
